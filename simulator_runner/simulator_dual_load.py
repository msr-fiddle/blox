...
class SimulatorRunner(simulator_pb2_grpc.SimServerServicer):
    ...
    def __init__(
        ...
        self.second_workload_times = 0
        self.second_workload_flag = False
        self.second_workload = None
    ...
    def GetConfig(self, request, context):
        ...
        try:
            ...
            self.second_workload = self._generate_workload_second(job_config)
    ...
    def GetJobs(self, request, context) -> rm_pb2.JsonResponse:
        ...
        while True:
            try:
                ...
                if new_job_dict["job_arrival_time"] > simulator_time:
                    # no more jobs from base workload
                    # now need to get more jobs for the new workload
                    if simulator_time % 14400 == 0:
                        # # double the number of jobs
                        # # current_number_of_jobs = len(job_to_run_dict)
                        # send load of 16 jobsa
                        self.second_workload_times += 300
                        self.second_workload_flag = True
                        while True:
                            new_job = self.second_workload.generate_next_job(
                                self.second_workload_times
                            )
                            new_job_dict = self._clean_sim_job_second_work(
                                new_job.__dict__
                            )
                            if (
                                ...
                                new_job_dict["job_arrival_time"] = simulator_time
                                job_to_run_dict[jcounter] = new_job_dict
                                jcount += 1
                            else:
                                break
                    if self.second_workload_flag:
                        # this is going to be tiny bit off
                        # but should still be okay I think ?
                        self.second_workload_times += 300
                        while True:
                            new_job = self.second_workload.generate_next_job(
                                self.second_workload_times
                            ...
                            if (
                                ...
                                # changing time back to simulator time
                                new_job_dict["job_arrival_time"] = simulator_time
                                job_to_run_dict[jcounter] = new_job_dict
                                jcount += 1
                            else:
                                break
                    if simulator_time % 21600 == 0:
                        # stop getting workloads
                        self.second_workload_flag = False

                        # for extra_jobs_to_add in range(16):
                        # new_job_deep_copy = copy.deepcopy(new_job_dict)
                        # new_job_deep_copy["job_arrival_time"] = simulator_time
                        # new_job_deep_copy["job_total_iteration"] = 10000
                        # new_job_deep_copy["job_duration"] = 10000
                        # job_to_run_dict[jcounter] = new_job_deep_copy
                        # jcounter += 1
                        # new_job_dict["job_id"] = self.workload.job_id
                        # self.workload.job_id += 1
    ...
    def _clean_sim_job_second_work(self, new_job: dict) -> dict:
        """
        Preprocesses the job for simulations.
        Cleans some fields and non serializable input
        """
        new_job_time = random.randint(600, 3600)
        new_job["job_total_iteration"] = new_job_time
        new_job["job_duration"] = new_job_time
        new_job["simulation"] = True
        new_job["submit_time"] = new_job["job_arrival_time"]
    ...
    def _generate_workload_second(self, workload_config):
        random.seed(self.random_seed)
        return Workload(
            self.cluster_job_log,
            jobs_per_hour=4 * workload_config["load"],  # second workload config 4 times
