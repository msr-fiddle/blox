...
from . import parse_philly_trace_full
from .utils import (
...
class Workload:
    def __init__(
        ...
        if self.workload_type == "philly":
            ...
            self.logger.info(
                "Total Jobs in Philly Trace: %s, Exponential=%s, multigpu=%s, debug_multi=%s",
                self.total_jobs,
                exponential,
                multigpu,
                debug_multi,
        ...
        self.logger.info(
            "Chosen split = {}% image, {}% lang, {}% speech. Mode={}".format(
                *self.model_class_split, str(self.model_assignment_option)
            )
        )
    ...
    def analyze_philly_trace(self):
        ...
        for job in self.jobs:
            ...
            j_q.put((job.job_arrival_time, 1))
            q.put((job.job_arrival_time + job.job_duration, -job.job_gpu_demand))
            j_q.put((job.job_arrival_time + job.job_duration, -1))
        while not q.empty():
