        ...
        for i, a in enumerate(j["attempts"]):
            ...
            if not sum_attempts or i == len(j["attempts"]) - 1:
                ...
                if sum_attempts and not exponential:
                    ...
                    assert (
                        total_iterations == duration_sum_seconds
                    ), "Job duration doesnt match trace"

                # Get jobs only in this range where demand > 512 GPUs
                if not exponential:
                    if (
                        arrival_time_seconds <= 3600 * 2400
                        or arrival_time_seconds >= 3600 * 3000
                    ):
                        continue
                ...
                if not exponential and len(jobs) == 8000:
                    ...
                    for job in jobs:
                        ...
                        logger.info(
                            "Job : {}, arr : {:.2f}, iter_time : {}, iters : {}, gpu : {}\n".format(
                                job.job_id,
                                job.job_arrival_time / 3600,
                                job.job_iteration_time,
                                job.job_total_iteration,
                                job.job_gpu_demand,
                            )
                        )
                    return jobs
