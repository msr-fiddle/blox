...
class DataSeries:
    ...
    def plot_step(
        self, path="./", mean=False, must_print=False, serv_id=None, metric=None
    ):
        plt.figure()
        df = self.get_df()
        ...
        if serv_id is not None or metric is not None:
            self.logger.info(
                "Mean {} for server {} = {}".format(
                    metric, serv_id, str(df[self.name[1]].mean())
                )
            )
        elif must_print:
            self.logger.info("Mean of GPU demand : %s", str(df[self.name[1]].mean()))
    ...
    def plot_cdf(self, path="./", prefix="job_completion"):
        ...
        self.logger.info("Mean of Trace Durations: %s", str(df[self.name[1]].mean()))
        df["cdf"] = df[self.name[1]].rank(method="average", pct=True)
        ax = df.sort_values(self.name[1]).plot(
            x=self.name[1],
            y="cdf",
            grid=True,
            linewidth=2,
...
class DataSeriesCollection:
    ...
    def plot_cdf(self):
        ...
        for group_name in grouped_dfs:
            ...
            for label_df in grouped_dfs[group_name]:
                ...
                if "%H:%M:%S" in df.columns[1]:
                    ...
                    plt.gca().xaxis.set_major_locator(
                        mdates.MinuteLocator(interval=600)
                    )
                self.logger.info("jct cdf for %s : %s", np.round(group_name, 1), label)
            ...
            plt.gcf().autofmt_xdate()
            plt.title(
                "CDF of JCT for load = " + str(np.round(group_name, 1)) + " jobs/hour"
            )
            plt.savefig(
                "cdf_jct_load_" + str(np.round(group_name, 1)) + ".png",
                bbox_inches="tight",
            )
            plt.close()

    def plot_mean(self, xlabel="Load (jobs/hour)", ylabel="Avg. JCT (hours)"):
        ...
        plt.savefig("avg_jct_vs_load.png")
        plt.close()

    def plot_weighted_mean(
        self, xlabel="Load (jobs/hour)", ylabel="Avg. GPU Demand (%)"
    ):
        grouped_dfs = self.group_by_index(index_id=0)
        plt.figure()
        for group_name in grouped_dfs:
