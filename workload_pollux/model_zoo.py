...
from task import Task, TaskName
...
class ModelZoo:
    ...
    def __init__(
        ...
        for key, val in self.model_zoo.items():
            self.logger.info(
                "{} : {}, {}, {}".format(
                    key, val.model_name, val.model_task, val.model_id
                )
            )
    ...
    def get_job_class(self):
        ...
        if self.assignment == ModelAssignment.OVERALL:
            ...
            sorted_tasks = sorted(
                self.tasks.items(), key=lambda x: x[1].overall_priority, reverse=True
            )
        elif self.assignment == ModelAssignment.RUNNABLE:
            sorted_tasks = sorted(
                self.tasks.items(), key=lambda x: x[1].runnable_priority, reverse=True
            )
    ...
    def create_default_models(self):
        ...
        # speech_options = []
        speech_options = ["deepspeech", "res50"]
