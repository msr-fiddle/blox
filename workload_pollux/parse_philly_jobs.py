...
def parse_jobs(
    ...
    for j in data:
        ...
        queueing_delay = 0
        name = "alexnet-0"                                   # Kaushik
        application = APPLICATIONS['alexnet']                # Kaushik
...
def parse_jobs_scheduler(
    ...
    csvfile = open(path, 'r')
    fieldnames = ("name","time","application","num_replicas","batch_size")
    ...
    for j in data[1:]:
        count += 1
        submitted_time = int(j["time"])
        arrival_time_seconds = int(j["time"])
        ...
        print(f"IN PARSE_JOBS_SCHEDULER: application={application}")
        jobs.append(
                    Job(
                        job_id,
                        arrival_time_seconds,
